...
export async function generateClassStudentPDFs(req: Request, res: Response) {
  try {
    ...
    if (!requester) return res.status(401).json({ success: false, msg: "Unauthorized" });

    const classModel = new ClassModel(pool);
    const classInfo = await classModel.getClassById(classId);
    if (!classInfo) return res.status(404).json({ success: false, msg: "Class not found" });
    ...
    const csModel = new ClassStudentModel(pool);
    let students = await csModel.getStudentsByClassId(classId);

    // If studentIds provided, filter
    const studentIds: number[] = Array.isArray(req.body.studentIds) ? req.body.studentIds.map(Number) : [];
    ...
    try {
      ...
      const studentGradeModel = new StudentGradeModel(pool);
      ...
      for (const s of students) {
        ...
        const gradeItems = grades.map((g: any) => ({ subject_id: g.subject_id, subject_name: g.subject_name ?? null, grade: g.grade }));

        // compute average if numeric grades present
        const numericGrades = grades.map((g: any) => Number(g.grade)).filter((v: any) => !Number.isNaN(v));
        const average = numericGrades.length > 0 ? numericGrades.reduce((a: number, b: number) => a + b, 0) / numericGrades.length : null;
      ...
      try {
        const activitySvc = new ActivityService(new ActivityModel(pool));
