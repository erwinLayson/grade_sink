...
function renderSubjectGradeRows(student) {
...
function renderPdfDocument(rows) {
    return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
function generateGradePDF(result) {
    const rows = result.map(renderStudentPage).join("");
    return renderPdfDocument(rows);
}
//# sourceMappingURL=generateGrade.pdf.js.map